...
export default function ExamAppLogo({ role }: ExamAppLogoProps) {
  return (
    <div
      className={`font-semibold flex items-center gap-2.5 font-geist-mono text-xl ${role === USER_ROLES.ADMIN ? "text-white" : "text-primary"}`}
    >
      <div className="relative h-10 w-10">
        <Folder
          fill={`${role === USER_ROLES.ADMIN ? "transparent" : "var(--primary)"}`}
          className="h-10 w-10"
        />

        <Code
          stroke={`${role === USER_ROLES.ADMIN ? "var(--primary)" : "white"}`}
